...
export default function App() {
  ...
  // ── Poll for remote changes every 30s (multi-user sync)
  useEffect(() => {
  ...
  const persist = useCallback((newAssets, newJOs, newCounter) => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        setSyncStatus('ok');
        addToast('✓ Saved & synced to cloud');
      } catch(e) {
        setSyncStatus('ok');
        addToast('⚠ Could not save to cloud', true);
      }
    }, 800);
  }, []);
  ...
  const syncLabel = syncStatus==='loading' ? '⟳ Loading...' : syncStatus==='saving' ? '☁ Saving...' : '✓ Synced';
